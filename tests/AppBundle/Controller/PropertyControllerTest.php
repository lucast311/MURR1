...
class PropertyControllerTest extends WebTestCase
{
    ...
    /**
     * Story 4L
     * Tests that a contact can be associated to a property
     */
    public function testAssociateContactSuccess()
    {
        $client = static::createClient(array(), array('PHP_AUTH_USER' => 'admin', 'PHP_AUTH_PW'   => 'password'));
        ...
        $form['appbundle_contactToProperty[contact]'] = 1;

        //submit the form
        $crawler = $client->submit($form);

        $this->assertContains("Ken, Kenson",$crawler->filter("#associatedContacts")->html());
    }

    /**
     * Story 4L
     * Tests that a contact cannot be associated with a property twice
     */
    public function testAssociatePropertyDuplicateFailure()
    {
        ...
        $this->assertContains("This contact is already associated to the this property", $client->getResponse()->getContent());
    }

    /**
     * Story 4L
     * Test that a contact association can be removed from a property
     */
    public function testAssociateContactRemoveSuccess()
    {
        $client = static::createClient(array(), array('PHP_AUTH_USER' => 'admin', 'PHP_AUTH_PW'   => 'password'));
        $client->followRedirect(true);
        ...
        $this->assertNotContains("Ken, Kenson",$crawler->filter("#associatedContacts")->html());
    }

   /**
    * Story 4L
    * Test that a user is redirected to the Contact Search page when the remove fails
    */
   public function testAssociateContactRemoveURLFailure()
   {
       // Create a client,
       $client = static::createClient(array(), array('PHP_AUTH_USER' => 'admin', 'PHP_AUTH_PW'   => 'password'));
       $client->followRedirects(true);
       ...
       $client->request('GET', "/property/removecontactfromproperty");
